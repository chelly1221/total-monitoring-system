...
- **설정 화면**: `SettingsWorkspace`의 한 화면 정리형. 왼쪽은 기능 표시·게이트 연결·데이터 관리, 오른쪽은 알람 사이렌 전용이다. 사이렌 목록은 가용 높이를 넘을 때만 내부 스크롤되며 제목과 추가 버튼은 고정된다. PC 클라이언트 토큰 설정은 제거했고, 서버 시작 시 기존 키를 삭제하며 API·백업 복원으로 다시 저장하지 않는다.
...
- 메인 장비 목록, 온습도 장비/그래프, UPS 장비/그래프는 `SortableGroup`과 `SortableCard`로 기존 영역 안에서 바로 드래그해 재정렬한다. 별도 배치 변경/완료 모드는 없다. 6px를 넘는 포인터 이동에서만 드래그를 시작하고 놓으면 즉시 저장한다. 일반 클릭과 수정키 클릭은 기존 상세보기 동작을 유지하며, 드래그 직후 클릭은 차단한다. 호버/포커스 시 작은 이동 핸들에서 Enter → 방향키 → Enter 및 Escape 취소도 지원한다.
