...
3.2.0부터 서버가 보낸 파일을 받아 저장하고(exe/msi는 자동 실행, V3 엔진 설치 파일은 `/S`로 조용히 설치) 진행률을 서버에 보고합니다. 파일은 설정 파일 옆 `received/` 폴더에 저장되며 SHA-256 검증을 통과한 파일만 실행합니다. 관리자 권한이 필요한 설치 파일은 이 PC에서 UAC 확인이 뜰 수 있습니다.
