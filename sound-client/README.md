...
- **화면 구성**: 흰 바탕에 포인트 컬러(CMYK 0·90·100·50 ≈ `#7F0D00`)를 쓰는 창(460×300)입니다. 별도 타이틀바 없이 탭 줄이 드래그 손잡이 역할을 하며 오른쪽 끝에 최소화·닫기 버튼이 있습니다. 글꼴은 Pretendard Variable을 앱에 내장해(`src/fonts/`, SIL OFL) 모든 화면에 동일하게 적용합니다. 첫 화면인 **상태** 탭은 예전 soundsense처럼 큰 글자로 "무음 / 소리 감지"만 보여주고, 나머지 기능은 **음소거**(자동 해제 시간·즉시 해제), **설정**(장비명·음소거 팝업 기본 시간·임계값·무음 판정 시간·자동 실행), **서버**(전송 대상 IP/포트·on/off 문자열·하트비트·탐지 포트), **정보**(서버 대상·전송 이력·PC 정보·버전) 탭에 있습니다. 모든 탭은 300px 높이 안에 스크롤 없이 들어갑니다. 타이틀바의 X는 트레이로 숨깁니다.
- **상태 아이콘**: 트레이 아이콘과 작업표시줄 창 아이콘이 상태에 따라 바뀝니다. 초록 = 정상(무음), 회색 = 음소거됨, 빨강 = 소리 감지. 음소거 카운트다운 중에는 아이콘 위에 남은 시간(분 단위, 1시간 이상은 "2h", 마지막 1분은 초 단위·주황색)이 숫자로 표시됩니다. 원본은 `src-tauri/icons/src/state-*.png`(여백 없이 잘라낸 정사각형)이고, 앱 아이콘(exe)은 초록 아이콘에서 `cargo tauri icon`으로 생성했습니다.
...
3.2.0부터 서버가 보낸 파일을 받아 저장하고(exe/msi는 자동 실행, 설치 프로그램 창이 이 PC 화면에 그대로 뜸) 진행률을 서버에 보고합니다. 파일은 설정 파일 옆 `received/` 폴더에 저장되며 SHA-256 검증을 통과한 파일만 실행합니다. 관리자 권한이 필요한 설치 파일은 이 PC에서 UAC 확인이 뜰 수 있습니다.

3.1.0부터 서버와 클라이언트의 인증 토큰·서명 기능을 제거했습니다. 기존 설정 파일의 토큰은 시작 시 자동으로 제거되며, PC 식별자·전송 대상·오디오 설정은 유지됩니다. 이전 버전에서 토큰을 사용하던 시설 PC도 새 클라이언트로 교체하면 별도 토큰 설정 없이 등록할 수 있습니다.
...
3.2.1에서는 메인 창과 음소거 팝업의 네이티브 그림자를 끕니다. 제목줄 없는 창에 그림자를 켜면 Windows가 좌우·아래쪽 프레임을 남길 수 있습니다. 이전의 `DWMWA_BORDER_COLOR` 처리만으로는 Windows 10을 해결할 수 없어, 창 생성 설정과 런타임 모두 `shadow=false`로 통일했습니다. 화면 내부의 회색 경계선은 유지됩니다.

실제 Windows 프레임 회귀 검사는 `src-tauri`에서 `cargo run --release --example window_frame_check`로 실행합니다. 메인 창과 음소거 팝업의 외곽 크기와 내용 영역 크기가 같아야 하며, 크기 변경 후에도 확인합니다. `-- --shadow-control`을 붙이면 기존 그림자 설정의 추가 프레임을 대조합니다. 검사 창은 숨김 상태이며 오디오·네트워크 탐지·자동 실행·방화벽 설정을 실행하지 않습니다.

수정된 서버에서 내려받은 `tms-soundsense.zip`을 시설 PC에도 다시 풀어 **3.2.1** 클라이언트로 교체해야 적용됩니다. 기존 `soundsense-settings.json`은 그대로 보관합니다.
...
- **설정**: 장비명, 감지 임계값, 무음 판정 시간(ms), 하트비트 주기(ms), 자동 뮤트 해제 시간, 탐지 포트, Windows 시작 시 자동 실행, 서버 전송 대상(읽기 전용 + `수동 설정` 펼침에서 IP/포트/on/off 직접 입력).
