...
- **화면 구성**: 흰 바탕에 포인트 컬러(CMYK 0·90·100·50 ≈ `#7F0D00`)를 쓰는 커스텀 타이틀바 창(460×300)입니다. 글꼴은 Pretendard Variable을 앱에 내장해(`src/fonts/`, SIL OFL) 모든 화면에 동일하게 적용합니다. 첫 화면인 **상태** 탭은 예전 soundsense처럼 큰 글자로 "무음 / 소리 감지"만 보여주고, 나머지 기능은 **뮤트**(자동 해제 시간·즉시 해제), **설정**(장비명·임계값·포트·토큰·자동 실행·수동 대상), **정보**(서버 대상·전송 이력·PC 정보) 탭에 있습니다. 타이틀바의 X는 트레이로 숨깁니다.
