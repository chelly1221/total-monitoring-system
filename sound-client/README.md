...
- **자동 뮤트 해제**: PC가 뮤트되면 카운트다운(1분/10분/30분/1시간/2시간/3시간/5시간, 기본 10분)을 시작하고, 만료되면 뮤트를 해제하고 마스터 볼륨을 100%로 올립니다. 남은 시간은 화면과 트레이 툴팁에 표시되며 "지금 해제" 버튼으로 즉시 해제할 수 있습니다.
...
- **화면 구성**: 흰 바탕에 포인트 컬러(CMYK 0·90·100·50 ≈ `#7F0D00`)를 쓰는 커스텀 타이틀바 창입니다. 첫 화면인 **상태** 탭은 예전 soundsense처럼 큰 글자로 "무음 / 소리 감지"만 보여주고, 나머지 기능은 **뮤트**(자동 해제 시간·즉시 해제), **설정**(장비명·임계값·포트·토큰·자동 실행·수동 대상), **정보**(서버 대상·전송 이력·PC 정보) 탭에 있습니다. 타이틀바의 X는 트레이로 숨깁니다.
