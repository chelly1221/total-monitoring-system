...
/// MAC of the first adapter that has a MAC and a non-loopback IPv4 address.
pub fn first_mac() -> Option<String> {
    enumerate()
        .into_iter()
        .find(|a| a.mac.is_some() && a.ipv4.iter().any(|ip| !ip.is_loopback()))
        .and_then(|a| a.mac)
}
