...
- 첫 실행 기본값은 제1레이더 PC의 운영 설정(`settings.json`/`ups2_settings.json`)과 같다: UPS 192.168.0.99/.98, 통합감시 서버 192.168.1.160 포트 1991/1990, 5초 갱신, 임계값(UPS#1 배터리 전압 200~500 V, UPS#2 입력·출력 전압 180~450 V, 배터리 전압 0~290 V 등). 화면은 원본처럼 UPS별 항목/값 표(UPS#1 24항목은 두 열로 나눔)와 오른쪽 이벤트 로그로 구성한다.
- 항목별 최소/최대 임계값, 낮음/높음/정상 복귀 로그, 출력·배터리 상태 변경 로그, SNMP 응답 없음 로그. 이벤트 로그는 유닛별 500줄.
